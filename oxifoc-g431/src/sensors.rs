...
pub mod hall {
    ...
    pub fn init_hall(
        ...
        // Seed the estimator with the boot-time hall state: the sector is
        // known from the pin levels before any edge arrives. Without this,
        // `sample()` is None until the rotor moves — on a sensored cold
        // start the first torque command would commutate via the open-loop
        // recovery override instead of the actual angle. A disconnected
        // cable (pull-ups read 0b111) surfaces immediately as an invalid
