...
| 2026-08-02 | Config-hygiene validation (~1 KB: boundary guards for every config group, detect request validation, derating step-cut) overflowed the g431 default by 840 B; paid for by moving core's per-section ISR profiling out of the DEFAULT profile into the new `isr-profiling` feature (bench profile keeps it; the isr/s totals stay everywhere) — default headroom now ~630 B. RAM side: the g431 ISR diagnostics atomics moved to `.ccmdata` (36 B; the 22K SRAM region was 4 B from full on the bench build) | net **−650 B** code in default |
