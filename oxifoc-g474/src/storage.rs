...
//! Uses the last 4KB of bank 2 (2 pages × 2KB) for non-blocking async operations.
//!
//! Flash layout (dual-bank mode):
//!   Bank 1: 0x08000000 - 0x0803FFFF (256KB) - Firmware
//!   Bank 2: 0x08040000 - 0x0807FFFF (256KB) - Available for storage
//!   Storage: 0x0807F000 - 0x0807FFFF (4KB) - Last 4KB of bank 2
...
/// Start address of storage region (offset from flash base 0x08000000)
/// Storage is in bank 2, last 4KB: 0x0807F000
/// Bank 2 starts at 0x08040000 (256KB offset)
const STORAGE_START: u32 = 0x7F000; // 508KB offset (last 4KB of 512KB)

/// Size of storage region in bytes (4KB = 2 pages × 2KB in dual-bank mode)
...
/// Async flash type alias - uses true async flash for non-blocking operations
...
///
/// Uses async flash operations on bank 2, allowing the CPU to continue
/// executing code from bank 1 during erase/write operations.
#[task]
pub async fn storage_worker(flash: AsyncFlash) {
    let buf = BUFFER.take();

    // Create storage configuration for bank 2 region
